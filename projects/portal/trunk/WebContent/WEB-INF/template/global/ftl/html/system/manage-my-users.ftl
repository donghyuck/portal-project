			...
			if( ! renderTo4.data("kendoGrid") ){	
					renderTo4.kendoGrid({
						...
						columns: [
							...
							{ field: "inherited", title: "그룹", width:100,  filterable: false, sortable: false, template:'#if (inherited){ #<input type="checkbox" class="k-checkbox" checked="checked" disabled="disabled"><label class="k-checkbox-label">&nbsp;</label>#}#' }, 
							{ field: "inherited", title: "개인", width:100,  filterable: false, sortable: false, template:'#<input type="checkbox" class="k-checkbox" #if (ownership){ #checked="checked" #}# ><label class="k-checkbox-label">&nbsp;</label>' }, 
							{ field: "ownership", title: "권한부여됨", width:100,  filterable: false, sortable: false }
						],
						dataBound:function(e){
			...
			if( ! renderTo.data("kendoGrid") ){	
					renderTo.kendoGrid({
						...
						columns: [
							...
							{ field: "name",  title: "그룹",  filterable: true, sortable: true, template: '<span class="#if (membership){ #text-primary# }else{ #text-default#}#"><i class="fa fa-users"></i></span> #: displayName #(#: name #) <div class="pull-right">#if (membership){ #<a href="\\#" class="btn btn-xs btn-labeled btn-danger k-grid-edit"><span class="btn-label icon fa fa-user-times"></span> 탈퇴</a>#}else{#<a href="\\#" class="btn btn-xs btn-labeled btn-success k-grid-edit"><span class="btn-label icon fa fa-user-plus"></span> 가입</a>#}#</div>' },
