					...
					<div class="container">
						<div class="row">
							...
							<#list navigator.parent.components as item >								
								<a class="btn btn-blue btn-sm <#if navigator.parent.components?seq_index_of(item) == 0>rounded-left<#elseif navigator.parent.components?seq_index_of(item) == (navigator.parent.components?size - 1)  >rounded-right</#if> <#if item.name ==  navigator.name >active</#if>" href="${item.page}">${ item.title }</a>					
							</#list>		
							</div>
							</div>							
