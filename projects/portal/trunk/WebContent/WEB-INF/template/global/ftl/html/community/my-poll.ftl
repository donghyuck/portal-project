							...
							if (!common.ui.exists(listview)) {								
								common.ui.grid(listview, {
									...
									columns:[{
										...
										field: 'optionText',
										title: "내용"},
										{ command: [ { name: "edit", template:'<a href="\\#" class="btn btn-sm btn-labeled btn-danger rounded k-grid-eidt"><span class="btn-label icon fa fa-trash"></span> 수</a>' }, { name: "destroy", template:'<a href="\\#" class="btn btn-sm btn-labeled btn-danger rounded k-grid-delete"><span class="btn-label icon fa fa-trash"></span> 삭제</a>' }],  title: "&nbsp;", width: 80 }				
									],
									editable: "inline"
								});
