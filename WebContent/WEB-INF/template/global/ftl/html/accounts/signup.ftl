								...
								<form role="form" id="signup-form" name="fm1" method="POST" accept-charset="utf-8" class="sky-form">
									...
									<fieldset <#if !action.user.anonymous >disabled</#if> style="padding-top:0;" >
										...
										<div class="form-group">
											...
											<input type="password" class="form-control" id="signupInputPassword1" name="signupInputPassword1"  placeholder="비밀번호" data-bind="value: form.password1" required data-required-msg="비밀번호를 입력하여 주십시오." pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}" validationMessage="비밀번호는 대문자/소문자, 숫자를 포합하는 6자이상 문자이여야 합니다.">
										</div>
										<div class="form-group">
