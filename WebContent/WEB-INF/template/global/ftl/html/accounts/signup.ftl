								...
								<form role="form" id="signup-form" name="fm1" method="POST" accept-charset="utf-8" class="sky-form">
									...
									<fieldset <#if !action.user.anonymous >disabled</#if> style="padding-top:0;" >
										...
										<div class="form-group">
											...
											<input type="password" class="form-control" id="signupInputPassword1" name="signupInputPassword1"  placeholder="비밀번호" data-bind="value: form.password1" required data-required-msg="비밀번호를 입력하여 주십시오." pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=]).{8,}" validationMessage="비밀번호는 대문자, 소문자, 숫자, 특수문자를 포합하는 8자이상 문자이여야 합니다.">
										</div>
										<div class="form-group">
											<label for="signupInputPassword2">비밀번호 확인</label>
											<input type="password" class="form-control" id="signupInputPassword2" name="signupInputPassword2"  placeholder="비밀번호 확인" data-matches="form.password1" data-bind="value: form.password2" required data-matches-msg="비밀번호가 일치하지 않습니다. 다시 입력하여 주세요." data-required-msg="비밀번호를 다시한번 입력하여 주십시오.">
										</div>									
										<section>
