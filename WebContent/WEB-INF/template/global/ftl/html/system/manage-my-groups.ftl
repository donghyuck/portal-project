		...
		function createGroupUserGrid(renderTo, data){
			if( ! common.ui.exists(renderTo)){	
				common.ui.grid(renderTo, {
					...
					columns: [
						{ title: '<input type="checkbox" id="selecte-all-groups" class="k-checkbox" /> <label class="k-checkbox-label" for="selecte-all-groups">&nbsp</label>', template: '<input type="checkbox" id="selected-groups" class="k-checkbox" /> <label class="k-checkbox-label" for="selected-groups">&nbsp</label>', width: 50},
